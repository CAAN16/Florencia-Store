...
function NuevoProductoContent() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (useUpload) {
        if (!imageFile) {
          setAlert({ type: 'error', message: 'Por favor selecciona o sube una imagen para el producto' });
      ...
      // Procesar tallas y colores de texto delimitado por comas a array de string
      ...
      // Crear el producto usando el productsService
      ...
      setAlert({ type: 'success', message: 'Producto agregado correctamente. Redirigiendo...' });
      ...
      console.error(error);
      setAlert({ type: 'error', message: error.message || 'Ocurrió un error al guardar el producto.' });
    } finally {
      setIsSubmitting(false);
    }
  };
            ...
            <div className="border border-border rounded-lg p-5 space-y-4">
              ...
              {imagePreview && (
                <div className="mt-4 flex flex-col items-center bg-muted/20 p-4 rounded-lg border border-border">
                  <p className="text-xs text-muted-foreground mb-2">Vista previa de la imagen:</p>
